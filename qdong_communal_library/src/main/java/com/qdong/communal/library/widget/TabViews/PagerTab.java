 ...
 * 通过{@link #setViewPager(ViewPager)}实现与viewPager关联,此viewPager的adpater要重写{@link android.support.v4.view.PagerAdapter#getPageTitle(int)}来给tab提供每个栏目的标题
...
public class PagerTab extends ViewGroup {
    ...
    private void addTextTab(final int position, CharSequence title) {
        ...
        if(mTabCount > 5) {
            int width = (int) ((DensityUtil.getDisplayWidth(mActivity) - (mDividerWidth * 4))/5);
